...
import {
  Plus, Trash2, ExternalLink, ChevronDown, Briefcase,
  MapPin, DollarSign, Calendar, StickyNote, X, Edit2,
...
// ─── Salary prediction helper ─────────────────────────────────────────────────
interface SalaryPrediction {
  display:         string;
  median_display:  string;
  currency:        string;
  skill_premium_pct: number;
  model_used:      boolean;
}

async function predictSalary(
  role: string, location: string, status: string,
): Promise<SalaryPrediction | null> {
  if (!role.trim()) return null;

  // Guess level from status (basic heuristic — user can always override)
  const level = 'mid';

  try {
    const res = await fetch('/api/ml/salary', {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({ role, location: location || 'Remote', level }),
      signal:  AbortSignal.timeout(8_000),
    });
    if (!res.ok) return null;
    const data = await res.json();
    if (data.fallback) return null;
    return data as SalaryPrediction;
  } catch {
    return null;
  }
}
...
function AppModal({ initial, onSave, onClose }: {
  ...
  async function handlePredict() {
    ...
    setPrediction(null);
    setPredError(false);
    const result = await predictSalary(f.role, f.location, f.status);
    setPredicting(false);
    if (result) {
      setPrediction(result);
    } else {
      setPredError(true);
      setTimeout(() => setPredError(false), 3000);
    }
  }

  function acceptPrediction() {
    if (!prediction) return;
    s('salary', prediction.display);
    setPrediction(null);
  }
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-100">
          <h3 className="text-sm font-semibold text-zinc-900">
            {initial.company ? 'Edit application' : 'New application'}
          </h3>
        ...
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Location"     value={f.location}    onChange={v => s('location', v)}     placeholder="London / Remote"/>
          ...
          {/* Salary field with AI predict button */}
          <div className="space-y-2">
            <label className="block text-xs font-medium text-zinc-600">Salary</label>
            <div className="flex gap-2">
              <input
                type="text" value={f.salary}
                onChange={e => { s('salary', e.target.value); setPrediction(null); }}
                placeholder="e.g. £60k–£80k or click ✨ to predict"
                className="flex-1 text-sm border border-zinc-200 rounded-xl px-3 py-2 bg-zinc-50 focus:outline-none focus:ring-2 focus:ring-blue-200 placeholder:text-zinc-400 transition-all"/>
              <button
                type="button"
                onClick={handlePredict}
                disabled={predicting || !f.role.trim()}
                title="Predict salary with ML model"
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium border transition-all disabled:opacity-40 bg-violet-50 border-violet-200 text-violet-700 hover:bg-violet-100 shrink-0">
                {predicting
            ...
            {/* Prediction result */}
            {prediction && (
              <div className="flex items-center justify-between p-3 rounded-xl border bg-violet-50 border-violet-200 animate-in fade-in duration-300">
                <div className="space-y-0.5">
                  <div className="flex items-center gap-1.5">
                    <TrendingUp size={11} className="text-violet-600"/>
                    <span className="text-[10px] font-semibold text-violet-700 uppercase tracking-wide">ML Salary Estimate</span>
                    {prediction.model_used && (
                      <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-violet-100 text-violet-600 border border-violet-200">trained model</span>
                    )}
                  </div>
                  <p className="text-sm font-bold text-violet-900">{prediction.display}</p>
                  <p className="text-[10px] text-violet-600">
                    Median: {prediction.median_display}
                    {prediction.skill_premium_pct > 0 && ` · +${prediction.skill_premium_pct}% skill premium`}
                  </p>
                </div>
                <button
                  onClick={acceptPrediction}
                  className="px-3 py-1.5 bg-violet-700 text-white rounded-lg text-xs font-medium hover:bg-violet-800 transition-all shrink-0">
                  Use this
                </button>
              </div>
            )}
            ...
            {predError && (
              <p className="text-[10px] text-red-500 px-1">
                ML service unavailable — start ml-service/start.bat
              </p>
            )}
          </div>
          ...
          <div>
            ...
            <textarea
              value={f.notes} onChange={e => s('notes', e.target.value)} rows={3}
...
function AppCard({ app, onEdit, onDelete, onMove }: {
  app: JobApplicationDoc; onEdit: () => void;
  onDelete: () => void; onMove: (s: AppStatus) => void;
}) {
  ...
  return (
    <div className="bg-white border border-zinc-200/80 rounded-xl p-3.5 shadow-sm hover:shadow-md transition-all space-y-2">
      <div className="flex items-start justify-between gap-2">
        ...
        <span className={`shrink-0 text-[9px] px-1.5 py-0.5 rounded-full border font-medium capitalize ${PRIORITY_STYLES[app.priority]}`}>
          {app.priority}
        </span>
      </div>
      ...
      {app.notes && (
        <p className="text-[10px] text-zinc-400 line-clamp-2 flex items-start gap-1">
          <StickyNote size={9} className="shrink-0 mt-0.5"/>{app.notes}
        </p>
      )}
      <div className="flex items-center gap-1 pt-1.5 border-t border-zinc-100">
        ...
        <div className="relative ml-auto">
          <button onClick={() => setMenu(v => !v)}
            className="flex items-center gap-0.5 px-2 py-1 text-[10px] text-zinc-500 hover:bg-zinc-100 rounded-lg transition-all">
            ...
            <div className="absolute bottom-full right-0 mb-1 bg-white border border-zinc-200 rounded-xl shadow-lg z-20 py-1 min-w-[110px]">
              {COLUMNS.filter(c => c.id !== app.status).map(c => (
                <button key={c.id} onClick={() => { onMove(c.id); setMenu(false); }}
                  className="w-full text-left px-3 py-1.5 text-[11px] text-zinc-600 hover:bg-zinc-50">
                  {c.label}
                </button>
              ))}
            </div>
...
function Chip({ icon, children }: { icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <span className="flex items-center gap-0.5 text-[9px] text-zinc-500 bg-zinc-100 px-1.5 py-0.5 rounded-full">
      {icon}{children}
    </span>
  );
}
...
export default function JobTracker() {
  ...
  const [modal, setModal]   = useState<
    { mode: 'add'; status: AppStatus } | { mode: 'edit'; app: JobApplicationDoc } | null
  >(null);
  ...
  if (loading) return (
    <div className="space-y-4">
      {Array(3).fill(0).map((_, i) => <div key={i} className="h-20 bg-zinc-100 rounded-2xl animate-pulse"/>)}
    </div>
  );
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-0.5">
            {apps.length} applications · {apps.filter(a => a.status === 'interview').length} interviews · {apps.filter(a => a.status === 'offer').length} offers
            <span className="ml-2 text-violet-500 font-medium">· ✨ AI salary prediction available</span>
          </p>
        </div>
        <button
          onClick={() => setModal({ mode: 'add', status: 'wishlist' })}
      ...
      {modal && (
        <AppModal
          initial={modal.mode === 'edit' ? modal.app : { ...BLANK, status: modal.status }}
          onSave={save}
