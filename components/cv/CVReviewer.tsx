...
import {
  Upload, Search, RefreshCw, CheckCircle, AlertCircle, XCircle,
  Check, Info, FileText, AlignLeft, Star, Sparkles, Cpu,
} from 'lucide-react';
...
// ─── Client-side fallback (instant, no API needed) ───────────────────────────
function analyzeCV(cvText: string, jd: string): AnalysisResult {
  ...
  const ats = Math.round(
    kwScore * 0.35 + experienceScore * 0.25 + skillsScore * 0.20 +
    formattingScore * 0.10 + summaryScore * 0.10
  );
  ...
  const sections: Section[] = [
    ...
    { name:'Formatting',       score:formattingScore, status:formattingScore>=80?'good':'warn',                         tip:isFormatted?'Clean layout detected. Ensure single-column format for ATS.':'Improve structure — use clear section headers.' },
  ];
...
function Ring({ score }: { score: number }) {
  ...
  return (
    <svg width="96" height="96" viewBox="0 0 96 96">
      ...
      <text x="48" y="52" textAnchor="middle" fontSize="18" fontWeight="800"
        fill={color} fontFamily="system-ui">{score}</text>
    </svg>
  );
}
...
function AIBadge({ loading }: { loading: boolean }) {
  if (loading) {
    return (
      <span className="flex items-center gap-1.5 text-[10px] px-2.5 py-1 rounded-full border bg-green-50 border-green-200 text-green-600 animate-pulse">
        <Cpu size={10}/> AI scoring…
      </span>
    );
  }
  return (
    <span className="flex items-center gap-1.5 text-[10px] px-2.5 py-1 rounded-full border bg-green-50 border-green-200 text-green-700 font-medium">
...
export default function CVReviewer() {
  ...
  async function handleAnalyze() {
    ...
    // 1. Run client-side instantly — show results right away
    const clientResult = analyzeCV(cvText, jd);
    setTimeout(() => {
    ...
    // 2. Call ML service in parallel — update results when it responds
    setMlLoading(true);
    try {
      const res = await fetch('/api/ml/ats', {
        method:  'POST',
      ...
      if (res.ok) {
        ...
        if (!ml.fallback) {
          // Convert ML response shape to our AnalysisResult shape
          const mlResult: AnalysisResult = {
            ats:           ml.overall_score,
            sections:      ml.sections.map((s: any) => ({
              name:   s.name,
              score:  s.score,
              status: s.status,
              tip:    s.tip,
            })),
            missing:       ml.missing_keywords ?? [],
            matched:       ml.matched_keywords ?? [],
            strengths:     ml.strengths ?? [],
            aiEnhanced:    true,
            semanticScore: ml.semantic_score,
            keywordScore:  ml.keyword_score,
            wordCount:     ml.word_count,
            verbCount:     ml.verb_count,
          };
          setResult(mlResult);
        }
      }
    } catch {
      // ML service unavailable — client-side result already showing, that's fine
    } finally {
      setMlLoading(false);
    }
  }
  ...
  if (step === 'result' && result) {
    const { ats, sections, missing, matched, strengths, aiEnhanced,
            semanticScore, keywordScore, wordCount, verbCount } = result;
    ...
    return (
      <div className="space-y-5 max-w-4xl">
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2">
              ...
              {(aiEnhanced || mlLoading) && <AIBadge loading={mlLoading}/>}
            </div>
            <p className="text-xs text-zinc-400 mt-0.5">
              {fileName || 'Your CV'} · {jd ? 'vs. job description' : 'general review'}
              {aiEnhanced && ' · powered by sentence-transformers'}
            </p>
          </div>
          <button
            onClick={() => { setStep('upload'); setResult(null); setMlLoading(false); }}
            className="text-xs px-3 py-1.5 border border-zinc-200 rounded-xl text-zinc-500 hover:bg-zinc-50 transition-all">
            Analyse another
          </button>
        </div>

        {/* Top row: score ring + mini stats + section bars */}
        <div className="grid grid-cols-3 gap-4">

          {/* ATS ring + mini stats */}
          <div className="bg-white border border-zinc-200/60 rounded-2xl p-5 flex flex-col items-center gap-3">
            <Ring score={ats}/>
            ...
            <span className={`text-xs px-3 py-1 rounded-full font-medium border ${
              ats >= 80 ? 'bg-emerald-50 text-emerald-700 border-emerald-200'
            : ats >= 60 ? 'bg-amber-50 text-amber-700 border-amber-200'
            :             'bg-red-50 text-red-700 border-red-200'}`}>
              {ats >= 80 ? 'Strong match' : ats >= 60 ? 'Needs work' : 'Poor match'}
            </span>

            {/* Mini stats (shown when AI enhanced) */}
            {aiEnhanced && (
              <div className="w-full space-y-1.5 pt-2 border-t border-zinc-100">
                {semanticScore !== undefined && (
                  <div className="flex justify-between text-[10px]">
                    <span className="text-zinc-400">Semantic match</span>
                    <span className="font-semibold text-violet-700">{semanticScore}%</span>
                  </div>
                )}
                {keywordScore !== undefined && (
                  <div className="flex justify-between text-[10px]">
                    <span className="text-zinc-400">Keyword match</span>
                    <span className="font-semibold text-blue-700">{keywordScore}%</span>
                  </div>
                )}
                {wordCount !== undefined && (
                  <div className="flex justify-between text-[10px]">
                    <span className="text-zinc-400">Word count</span>
                    <span className={`font-semibold ${wordCount < 150 ? 'text-red-600' : wordCount > 1200 ? 'text-amber-600' : 'text-emerald-600'}`}>
                      {wordCount}
                    </span>
                  </div>
                )}
                {verbCount !== undefined && (
                  <div className="flex justify-between text-[10px]">
                    ...
                    <span className={`font-semibold ${verbCount >= 5 ? 'text-emerald-600' : 'text-amber-600'}`}>
                      {verbCount} detected
                    </span>
                  </div>
                )}
          ...
          <div className="col-span-2 bg-white border border-zinc-200/60 rounded-2xl p-5 space-y-3">
            ...
            {sections.map(s => (
              <div key={s.name} className="flex items-center gap-3">
                <span className="w-36 text-xs text-zinc-600 shrink-0">{s.name}</span>
                <div className="flex-1 h-1.5 bg-zinc-100 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-700 ${
                      s.status === 'good' ? 'bg-emerald-500' : s.status === 'warn' ? 'bg-amber-500' : 'bg-red-500'
                    }`}
                    style={{ width: `${s.score}%` }}
                  />
                </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-white border border-zinc-200/60 rounded-2xl p-5 space-y-2.5">
            <h3 className="text-sm font-semibold text-zinc-900 flex items-center gap-2">
              <AlertCircle size={14} className="text-amber-500"/>Improvements
            </h3>
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        ...
        <p className="text-xs text-zinc-400 mt-0.5">
          ATS score, keyword analysis &amp; section-by-section feedback.
          <span className="ml-1 text-violet-600 font-medium">AI-enhanced when ML service is running.</span>
        </p>
      </div>
      ...
      <div className="bg-white border border-zinc-200/60 rounded-2xl p-6 space-y-4">
        <h3 className="text-sm font-medium text-zinc-900 flex items-center gap-2">
          <Upload size={14} className="text-blue-600"/>Upload your CV
        </h3>
        <div
          onDrop={handleDrop}
          ...
          className={`border-2 border-dashed rounded-xl p-8 flex flex-col items-center justify-center gap-3 cursor-pointer transition-all ${
            fileName ? 'border-emerald-300 bg-emerald-50' : 'border-zinc-200 hover:border-blue-300 hover:bg-blue-50/30'
          }`}>
          {fileName
        ...
        <div className="space-y-2">
          ...
          <textarea
            value={cvText}
            onChange={e => { setCvText(e.target.value); if (e.target.value) setFileName('Pasted text'); }}
      ...
      <div className="bg-white border border-zinc-200/60 rounded-2xl p-6 space-y-3">
        <h3 className="text-sm font-medium text-zinc-900 flex items-center gap-2">
          ...
          <span className="text-[10px] text-zinc-400">(optional — greatly improves AI scoring)</span>
        </h3>
        <textarea
