...
import { Mail, Lock, User, ArrowRight, Video, Trophy, Users, AlertCircle } from 'lucide-react';
      ...
      <div className="bg-black rounded-4xl ml-2 p-16 flex-col justify-center relative overflow-hidden hidden lg:flex h-full">
        ...
        <div className="relative z-10 p-4 space-y-3 max-w-md mx-auto w-full">
          ...
          <div className="bg-white rounded-[32px] p-5 shadow-2xl transform rotate-1 hover:rotate-0 transition-all duration-500 border border-zinc-100 relative group">
            <span className="absolute top-4 right-4 bg-blue-600 text-white text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-wider z-10">Simulation</span>
            <div className="relative aspect-[4/3] rounded-2xl overflow-hidden mb-4 bg-zinc-950">
              <img src="https://images.unsplash.com/photo-1551836022-d5d88e9218df?auto=format&fit=crop&w=600&q=80" alt="AI Interview" className="w-full h-full object-cover opacity-85 group-hover:scale-105 transition-transform duration-700" />
          ...
          <div className="bg-blue-600 p-4 mb-3 rounded-2xl shadow-xl transform -rotate-2 hover:rotate-0 transition-all duration-300 max-w-[280px] -translate-y-8 -translate-x-6">
            <div className="flex items-center gap-2.5 mb-3">
              <div className="p-1.5 bg-white/20 rounded-lg text-white"><Users size={16} /></div>
              <p className="text-xs font-black text-white">Happy Users: <span>2K+</span></p>
            </div>
            <div className="flex items-center justify-between pt-2 border-t border-white/20">
              <span className="text-[10px] text-white/80 font-bold tracking-tight">Placed at top companies</span>
              <div className="flex gap-2 items-center">
                <img src="https://upload.wikimedia.org/wikipedia/commons/2/2f/Google_2015_logo.svg" alt="Google" className="h-3 brightness-0 invert" />
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white px-8 md:px-16 lg:px-24 flex flex-col justify-center h-full">
        <div className="max-w-md w-full mx-auto space-y-6">
          <div>
            ...
            <p className="text-blue-600 font-bold text-xs tracking-wide uppercase mb-1">Get Started</p>
            <h1 className="text-zinc-900 text-4xl font-black leading-none tracking-tight">CREATE YOUR ACCOUNT</h1>
            <p className="text-zinc-500 text-sm mt-2 font-medium">Join Intervue today and unlock interactive AI interview simulations.</p>
          </div>
          ...
          <form className="space-y-3.5" onSubmit={handleSignUp}>
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? 'Creating account…' : <><span>Get Started</span><ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" /></>}
            </button>
          ...
          <p className="text-center text-zinc-500 text-xs font-medium">
            ...
            <Link href="/sign-in" className="text-blue-600 font-bold hover:underline ml-1">Login</Link>
          </p>
        </div>
      </div>
    </div>
